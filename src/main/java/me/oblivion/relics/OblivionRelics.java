...
public final class OblivionRelics extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        relicManager = new RelicManager(playerDataManager, relicItem);
        ...
        if (getCommand("relicgive") != null) {
            getCommand("relicgive").setExecutor(
                    new RelicGiveCommand(relicManager)
            );
        }

        if (getCommand("relicremove") != null) {
            getCommand("relicremove").setExecutor(
                    new RelicRemoveCommand(relicManager)
            );
        }
        ...
        if (getCommand("relicinfo") != null) {
            getCommand("relicinfo").setExecutor(relicInfoGUI);
        }
        ...
        getServer().getPluginManager().registerEvents(
                new UniversalAbilityListener(relicAbilityEngine),
                this
        );
        ...
        getLogger().info("Relic must be held in main hand to use abilities.");
        getLogger().info("Relic drop/container protection loaded.");
        getLogger().info("AutoCrit duration: 8 seconds.");
        getLogger().info("Phase Shift duration: 8 seconds.");
        getLogger().info("================================");
    }
    ...
    public EchoFlask getEchoFlask() { return echoFlask; }
    public TrustManager getTrustManager() { return trustManager; }
    public RelicItem getRelicItem() { return relicItem; }
    public RelicManager getRelicManager() { return relicManager; }
    public RelicAbilityEngine getRelicAbilityEngine() { return relicAbilityEngine; }
    public AbilityActionBar getAbilityActionBar() { return abilityActionBar; }
    public RelicStartManager getRelicStartManager() { return relicStartManager; }
    public RelicInfoGUI getRelicInfoGUI() { return relicInfoGUI; }
}
